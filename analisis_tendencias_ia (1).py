...
class AgenteTendenciasMercado:
    ...
    # Métodos asíncronos
    #
    # Para adherirnos al patrón de agentes con strands, definimos un método que
    # ejecuta todas las funciones de análisis en paralelo.  Utilizamos
    # `asyncio.to_thread` para delegar la ejecución de los métodos
    # sincrónicos existentes a hilos de ejecución en segundo plano.  De esta
    # forma cada tarea de análisis se comporta como un agente independiente
    # (o «strand») que colabora para generar el diccionario de tendencias.
    async def analizar_todo_concurrente(self) -> None:
        """Ejecuta todos los análisis de tendencias de forma concurrente.

        Este método lanza las funciones de análisis temporal, institucional,
        por programa, por sector y por área de conocimiento en hilos
        separados.  Espera a que todas finalicen antes de continuar.  Los
        resultados se almacenan en el atributo `self.tendencias` como en los
        métodos individuales.  Si alguna tarea falla, la excepción se
        propaga y debe ser gestionada por quien invoque la corrutina.
        """
        tareas = [
        ...
        # Ejecutar las tareas concurrently y esperar a que todas finalicen
...
class AgenteVisualizacion:
    ...
    # Método asíncrono
    #
    # Para mantener la coherencia con el enfoque basado en «strands», se
    # proporciona un método que genera todas las gráficas de forma
    # concurrente.  Cada llamada a una función de visualización se delega a
    # un hilo a través de `asyncio.to_thread`.  Al finalizar, se genera el
    # dashboard consolidado.  Este método se puede invocar mediante
    # `await` desde `asyncio.run` para permitir que otras tareas se ejecuten
    # en paralelo.
    async def crear_todas_graficas(self, top_n: int = 10) -> None:
        """Genera todas las gráficas de manera concurrente y luego un dashboard.

        Params:
            top_n (int): número de instituciones a mostrar en la gráfica
                "Top Instituciones".
        """
        tareas = [
            asyncio.to_thread(self.crear_grafica_tendencia_temporal),
            # Pasamos top_n como argumento posicional a la función que lo espera
        ...
        await asyncio.gather(*tareas)
        # Generar el dashboard completo una vez que las gráficas básicas estén listas
...
def main():
    ...
    # Ejecutar todas las tendencias en paralelo utilizando asyncio.  Con esto
    # replicamos el comportamiento basado en agentes y strands; cada
    # subanálisis se procesa en un hilo separado.  Si alguna tarea
    # produce un error, la excepción se propagará y será visible en la
    # consola.
    ...
    # Generar todas las visualizaciones en paralelo usando asyncio para
    # aprovechar los hilos.  Esto produce todas las imágenes y luego el
    # dashboard final.  Al igual que en la fase de análisis, cualquier
    # excepción se propaga a la consola.
